...
export class MemberMergingService {
  ...
  async preview(input: Omit<MergeMembersInput, 'actorUid' | 'actorEmail'>): Promise<MergePreview> {
    const { keep, drop } = await this.loadAndValidate(input.tenantId, input.keepId, input.dropId);

    // Use filters to get accurate counts. Limit 0 returns just the totals.
    const [txs, pledges] = await Promise.all([
      this.transactionService.getAll(input.tenantId, { memberId: drop.id, limit: 0 }),
      this.pledgeService.getAll(input.tenantId, { memberId: drop.id, limit: 0 }),
    ]);
  ...
  async merge(input: MergeMembersInput): Promise<MergeResult> {
    ...
    if (patch.userId && merged.userId && merged.userId !== keep.userId) {
      const linkedUser = await this.userService.findById(merged.userId as string);
      if (linkedUser) await this.userClaims.refreshFor(linkedUser.firebaseUid);
    }
