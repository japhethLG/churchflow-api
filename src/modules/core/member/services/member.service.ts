...
import type { MemberListResult } from '../repository/member.repository';
...
export class MemberService {
  ...
  async getAll(tenantId: string, filters?: MemberFilters): Promise<MemberListResult> {
    return this.memberRepository.findAll(tenantId, filters ?? {});
  }

  async update(tenantId: string, id: string, data: UpdateMemberInput): Promise<Member> {
  ...
  async linkUser(tenantId: string, id: string, userId: string): Promise<Member> {
    await this.getById(tenantId, id);
