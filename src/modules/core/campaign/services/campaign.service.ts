...
import type { CampaignListResult } from '../repository/campaign.repository';
...
export class CampaignService {
  ...
  async getAll(tenantId: string, filters?: CampaignFilters): Promise<CampaignListResult> {
    return this.campaignRepository.findAll(tenantId, filters ?? {});
  }

  async update(tenantId: string, id: string, data: UpdateCampaignInput): Promise<Campaign> {
  ...
  async restore(tenantId: string, id: string): Promise<Campaign> {
    const existing = await this.campaignRepository.findByIdIncludingDeleted(tenantId, id);
