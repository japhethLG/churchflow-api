...
export class PledgeFeatureService {
  ...
  async create(
    ...
    const effectiveMemberId = this.resolveMemberIdForCreate(tenant, data.memberId);
    ...
    if (data.campaignItemId) {
      const item = await this.campaignItemService.getById(tenant.tenantId, data.campaignItemId);
