...
export class TenantController {
  ...
  async create(
    ...
    return this.tenantFeatureService.create(user, body) as unknown as Promise<TenantResponseDto>;
  }
  ...
  @ApiParam({ name: 'tenantId', description: 'Tenant UUID (slug not supported for soft-deleted rows)' })
  @ApiOkResponse({ type: TenantResponseDto })
  async restore(
    @CurrentUser() user: AuthUser,
    @Param('tenantId') id: string,
  ): Promise<TenantResponseDto> {
    return this.tenantFeatureService.restore(user, id) as unknown as Promise<TenantResponseDto>;
  }
}
