...
export class TransactionListMetaDto extends MetaDto {
  @Expose()
  @ApiProperty({ example: AMOUNT_EXAMPLE, description: 'Sum of amounts in the current filter' })
  sum!: number;
}
...
export class TransactionSummaryResponseDto {
  ...
  @Expose()
  @ApiProperty({ example: 42, description: 'Number of transactions in the window' })
  count!: number;
